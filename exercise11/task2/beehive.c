...
void cleanup(flower_t* field, bee_data_t* data, pthread_t* thread, pthread_mutex_t* field_mutex,
             pthread_mutex_t* queue_mutex, pthread_barrier_t* barrier) {
	free(field);
	free(data);
	free(thread);
...
int main(int argc, char const* argv[]) {
	...
	for (int i = 0; i < number_of_bees; i++) {
		...
		if (pthread_create(&bees[i], NULL, thread_func, &bee_data[i]) != 0) {
			...
			cleanup(global_flower_field, bee_data, bees, &queue_mutex, &field_mutex, &barrier);
			exit(EXIT_FAILURE);
		}
	...
	cleanup(global_flower_field, bee_data, bees, &field_mutex, &queue_mutex, &barrier);
	return EXIT_SUCCESS;
}